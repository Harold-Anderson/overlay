...
DESCRIPTION="Linux ${K_BRANCH_ID} kernel sources with Con Kolivas' patchset and Gentoo's genpatches"
...
CK_INCP_URI="http://ck.kolivas.org/patches/muqss/4.0/4.8/Incremental/"
CK_INCP_LIST="4.8-muqss107-108.patch"
...
SRC_URI="${KERNEL_URI} ${LX_INCP_URI} ${GENPATCHES_URI} ${ARCH_URI} ${CK_INCP_URI}/${CK_INCP_LIST} ${CK_URI}
		       ${MUQSS_URL}/${MUQSS_FILE}"
#	!bfsonly? ( ${CK_URI} )
#	bfsonly? ( ${BFS_URI} )"
...
if use muqss ; then
	epatch "${DISTDIR}/${MUQSS_FILE}"
	epatch "${DISTDIR}/${CK_INCP_LIST}"
fi
